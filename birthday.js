...
function showWish(on){
  ...
  if (memoriesEl) memoriesEl.classList.toggle('is-in', on);
}
...
/* ============================================================
   BACKGROUND MUSIC CONTROLLER ("I Think They Call This Love")
   ============================================================ */
...
function setMusicUI(playing){
  ...
  if (musicTitle) {
    musicTitle.textContent = playing ? 'Pause Audio' : 'Play Audio';
  }
  if (musicArtist) {
    musicArtist.textContent = playing ? 'Playing: I Think They Call This Love' : 'I Think They Call This Love';
  }
  if (musicToggle) {
    musicToggle.setAttribute('aria-label', playing ? 'Pause Audio' : 'Play Audio');
  }
}
...
if (bgMusic){
  ...
  bgMusic.addEventListener('error', (e) => {
    console.error('Audio load/playback error:', bgMusic.error, e);
  });
}
...
/* ============================================================
   POLAROID MEMORIES & LIGHTBOX CONTROLLER
   ============================================================ */
const photosData = [
  { imgId: 'photoImg0', title: 'Forever by your side ♥' },
  { imgId: 'photoImg1', title: 'You are my whole world ♥' },
  { imgId: 'photoImg2', title: 'Every adventure with you ✨' },
];
...
try {
  localStorage.removeItem('bday_custom_photos');
} catch (_) {}

function openLightbox(idx){
  currentPhotoIdx = idx;
  const currentImg = $(`photoImg${idx}`);
  if (!currentImg) return;
  lightboxImg.src = currentImg.src;
  lightboxCaption.textContent = photosData[idx].title;
  lightboxCounter.textContent = `${idx + 1} / ${photosData.length}`;
...
function navLightbox(delta){
  currentPhotoIdx = (currentPhotoIdx + delta + photosData.length) % photosData.length;
  openLightbox(currentPhotoIdx);
}

document.querySelectorAll('.polaroid').forEach((p) => {
  p.addEventListener('click', () => {
    const idx = parseInt(p.getAttribute('data-index'), 10) || 0;
    openLightbox(idx);
  });
